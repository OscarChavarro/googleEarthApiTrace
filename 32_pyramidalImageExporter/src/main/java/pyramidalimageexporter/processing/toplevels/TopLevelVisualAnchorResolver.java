...
final class TopLevelVisualAnchorResolver {
    ...
    Map<String, String> resolve(List<MatrixLayer> topLayers, List<MatrixLayer> importedLayers) {
        ...
        List<MatrixLayerTile> topCells = topLayers.stream()
        ...
        for (MatrixLayer imported : importedLayers) {
            AnchorChoice choice = chooseAnchor(imported, topCells);
            if (choice == null) {
                continue;
            }
    ...
    private AnchorChoice chooseAnchor(MatrixLayer imported, List<MatrixLayerTile> topCells) {
        if (imported == null || imported.getTiles() == null || imported.getTiles().isEmpty()) {
            return null;
        }
        Map<Anchor, Integer> votes = new LinkedHashMap<>();
        int accepted = 0;
        for (MatrixLayerTile probe : evenlySpaced(imported.getTiles(), MAX_PROBES)) {
            MatchPair pair = bestTwoMatches(probe, topCells);
            if (!confident(pair)) {
                continue;
            }
            int level = pair.best().cell().getId().length() - 1;
            int side = 1 << level;
            Anchor anchor = new Anchor(
                level,
                pair.best().cell().getI() - probe.getI(),
                Math.floorMod(pair.best().cell().getJ() - probe.getJ(), side)
            );
    ...
    private MatchPair bestTwoMatches(MatrixLayerTile probe, List<MatrixLayerTile> topCells) {
        ...
        for (MatrixLayerTile cell : topCells) {
            ...
            Match candidate = new Match(cell, rmse(probeImage, cellImage, cell));
    ...
    private static double rmse(BufferedImage probe, BufferedImage source, MatrixLayerTile cell) {
        double sum = 0.0;
        long count = 0L;
        for (int y = 0; y < SAMPLE_SIDE; y++) {
            double fy = (y + 0.5) / SAMPLE_SIDE;
            int probeY = pixelAt(fy, probe.getHeight());
            double v = cell.getTexV1() - fy * (cell.getTexV1() - cell.getTexV0());
            ...
            for (int x = 0; x < SAMPLE_SIDE; x++) {
                ...
                double u = cell.getTexU0() + fx * (cell.getTexU1() - cell.getTexU0());
    ...
    private static <T> List<T> evenlySpaced(List<T> values, int limit) {
        ...
        for (int i = 0; i < limit; i++) {
            selected.add(values.get(i * (values.size() - 1) / (limit - 1)));
        }
        return selected;
    }
    ...
    private record Match(MatrixLayerTile cell, double rmse) {}
