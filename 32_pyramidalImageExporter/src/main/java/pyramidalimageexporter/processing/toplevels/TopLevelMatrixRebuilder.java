 ...
 */
public final class TopLevelMatrixRebuilder {
    ...
    public List<MatrixLayer> importLayers(TopLevelTilesCatalog topLevelTiles) {
        if (topLevelTiles == null || topLevelTiles.getByStripId() == null || topLevelTiles.getByStripId().isEmpty()) {
            System.out.println("TopLevelMatrixRebuilder: no top-level tiles to import.");
            return List.of();
        }
        ...
        if (stripWorldOriginByStripId.isEmpty()) {
            System.out.println(
                "TopLevelMatrixRebuilder: no strip has a whole-world appearance (texCoord size 1/"
        ...
        System.out.println(
            "TopLevelMatrixRebuilder: anchored strips=" + stripWorldOriginByStripId.size()
        ...
        System.out.println("TopLevelMatrixRebuilder: levels imported=" + out.size());
        return out;
    }
     ...
     */
    public Map<String, String> catalogedQuadPathsByImagePath(TopLevelTilesCatalog topLevelTiles) {
        if (topLevelTiles == null || topLevelTiles.getByStripId() == null || topLevelTiles.getByStripId().isEmpty()) {
            return Map.of();
        }
        Map<String, double[]> stripWorldOriginByStripId = computeStripWorldOrigins(topLevelTiles);
     ...
     */
    private static Map<String, double[]> computeStripWorldOrigins(TopLevelTilesCatalog topLevelTiles) {
        Map<String, double[]> out = new LinkedHashMap<>();
        for (Map.Entry<String, TopLevelTilesCatalog.TopLevelTile> entry : topLevelTiles.getByStripId().entrySet()) {
            TopLevelTilesCatalog.TopLevelTile tile = entry.getValue();
            if (tile == null || tile.getAppearances() == null) {
                continue;
            }
            for (TopLevelTilesCatalog.FrameAppearance appearance : tile.getAppearances()) {
                TopLevelTilesCatalog.TexCoord texCoord = appearance == null ? null : appearance.getTexCoord();
                if (texCoord == null) {
                    continue;
                }
    ...
    private static Map<String, ImageTile> catalogImages(
        TopLevelTilesCatalog topLevelTiles,
        ...
        for (Map.Entry<String, Map<ImageTileVote, Integer>> entry : votesByImagePath.entrySet()) {
            ...
            if (bestCount < totalCount) {
                System.out.println(
                    "TopLevelMatrixRebuilder: image " + entry.getKey()
    ...
    private static ImageTileVote toImageTileVote(TopLevelTilesCatalog.FrameAppearance appearance, double[] stripWorldOrigin) {
        if (appearance == null || appearance.getImagePath() == null || appearance.getImagePath().isBlank()) {
            return null;
        }
        TopLevelTilesCatalog.TexCoord texCoord = appearance.getTexCoord();
        if (texCoord == null) {
            return null;
        }
    ...
    private static MatrixLayer buildLayer(int level, Map<String, ImageTile> imagesByCellKey) {
        ...
        System.out.println(
            "TopLevelMatrixRebuilder: level " + level
                + " matrix=" + side + "x" + side
