...
public final class Jogl4QuadtreeRenderer {
    ...
    public void draw(
        ...
        drawCommands(
            gl2,
            instance,
            QuadtreeDrawPlanner.select(instance, camera, relativeScale),
            wires,
            loader,
            false,
            discoveryService
    ...
    public void drawAll(GL2 gl2, PyramidalImageInstance instance, double relativeScale, boolean wires, TileImageLoader loader) {
        drawCommands(gl2, instance, QuadtreeDrawPlanner.selectAllLeaves(instance, relativeScale), wires, loader, true, null);
    }
    ...
    private void drawCommands(
        ...
        for (DrawCommand command : commands) {
            boolean drawable = drawNode(gl2, command, loader, synchronous);
            if (drawable && discoveryService != null) {
    ...
    private boolean drawNode(GL2 gl2, DrawCommand command, TileImageLoader loader, boolean synchronous) {
        QuadtreeNode node = command.node();
        Texture texture = null;
        QuadtreeNode textureOwner = null;

        if (node.getTileFile() != null) {
            texture = textureCache.acquire(gl2, node.getTileFile(), loader, synchronous);
            textureOwner = node;
        }
        if (texture == null) {
